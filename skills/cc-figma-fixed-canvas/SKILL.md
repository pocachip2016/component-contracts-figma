...
4. **New — referenceAssets branching (M1 experimental feature)**:
...
5. Add a text label showing the slot id: `child.characters = slot.id` (visible only if image fill is not applied; if fill is applied, label remains in place but may be obscured)
6. **Fill color**:
