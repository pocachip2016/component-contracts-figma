...
4. **referenceAssets branching (M1 experimental feature)**:
   ...
   - **Production default for `scaleMode`: `FILL`** — applies to every `upload_assets` call that fills an image slot with real content, whether via the referenceAssets path above or a later manual nodeId-targeted upload (post-generation asset swap). Decided over `FIT` because production rendering already crops rather than letterboxes (e.g. the LOGO slot's `object-left` crop confirmed in `GuxHorizontalPosterTitleOverlay`) — `FILL` keeps the Figma preview visually consistent with actual runtime output. Known gap: Figma's `FILL` crop alignment is centered, not left-aligned like `object-left`, so a wide-aspect asset (e.g. LOGO 600×160 into a 360×160 slot) may crop differently in Figma preview vs. runtime — acceptable since Figma output here is design-time preview only, not the rendering source of truth.
